...
int MaxSumDifference(int a[], int n)
{
    ...
    // In this loop first we will insert one smallest element not entered till that time in final sequence and then enter a highest element
    // (not entered till that time) in final sequence so that we have large difference value. This process is repeated till all array has 
    //completely entered in sequence.Here, we have loop till n/2 because we are inserting two elements at a time in loop.
    for (int start= 0,end=n-1; start<=end; start++,end--) {
        finalSequence.push_back(a[start]);
        finalSequence.push_back(a[end]);
    }
